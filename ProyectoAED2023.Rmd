...
funestad <- function(x){
  ...
  res <- c("Media" = Media, "SD" = SD, "Mediana" = Mediana, "IQR" = IQR)
  return(res)
}
TablaEsta <- ValenBisi %>% group_by(tramo_horario) %>% 
  summarize(across(starts_with("num"), funestad)) %>% 
  mutate(numbre_Esta = rep(c("Media", "SD", "Mediana", "IQR"), each = 1)) %>%
...
#-----------------------------------------------
barplot_dev <- ggplot(df_top10_dev, aes(x = tramo_horario, y = numero_de_devoluciones, fill = numero_de_devoluciones, text = paste("Número de devoluciones: ", numero_de_devoluciones, "<br>Estación: ", estacion))) +
...
barplot_pres <- ggplot(df_top10_pres, aes(x = tramo_horario, y = numero_de_prestamos, fill = numero_de_prestamos, text = paste("Número de préstamos: ", numero_de_prestamos, "<br>Estación: ", estacion))) +
  geom_bar(stat = "identity") +
  labs(title = "Préstamos por hora", x = "Tramo Horario", y = "Préstamos") +
  scale_fill_gradient(low = "#66c2a5", high = "#fc8d62", name = "numero_de_prestamos") +
  theme_minimal() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
