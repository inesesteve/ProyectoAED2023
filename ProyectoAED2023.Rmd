...
```{r}
#Valores faltantes por columnas: vfalt
...
```
Vemos que hay 9 estaciones que no tienen registro alguno en determinados tramos horarios. La suposición es por que no han alquilado ni devuelto ninguna bicicleta durante esos tramos ningún día del año. Un caso curioso es el de la estación ``298_TALLER`` que podemos intuir que abren de 6 a 15.
...
funestad <- function(x){
  ...
  res <- c("Maximo" = Max, "Minimo" = Min, "Media" = Media, "SD" = SD, "Mediana" = Mediana, "IQR" = IQR, "Total" = Total)
  return(res)
}
TablaEsta <- ValenBisi %>% group_by(tramo_horario) %>% 
  summarize(across(starts_with("num"), funestad)) %>% 
  mutate(numbre_Esta = rep(c("Maximo", "Minimo", "Media", "SD", "Mediana", "IQR", "Total"), each = 1)) %>%
  pivot_wider(names_from = numbre_Esta, values_from = starts_with("nume")) %>%
  mutate(Balance_Total = numero_de_prestamos_Total - numero_de_devoluciones_Total)
TablaEsta
```
...
interactive_boxplot <- plot_ly(ValenBisi, x = ~get(initial_x),
                               y = ~numero_de_devoluciones, type = "box", text = ~estacion, jitter = 0.3,
                               marker = list(color = "rgba(255, 0, 0, 0.5)"), fillcolor = "rgba(255,0,0,0.1)",
...
interactive_boxplot2 <- plot_ly(ValenBisi, x = ~get(initial_x), y = ~numero_de_prestamos, type = "box", text = ~estacion, jitter = 0.3,
                              marker = list(color = "rgba(0, 0, 255, 0.5)"), fillcolor = "rgba(0,0,255,0.1)",
...
outbox_pres <- list()
for (i in 1:24){
outbox_dev[i] <- list(reglaboxplot(ValenBisi_por_tramo[[i]]$numero_de_devoluciones))
outbox_pres[i] <- list(reglaboxplot(ValenBisi_por_tramo[[i]]$numero_de_prestamos))
}
```
 * También usamos la regla 3 sigma, el identificador de Hampel o la regla de los percentiles para comprobar cuál es el identificador más robusto.
...
out3sig_pres <- list()
for (i in 1:24){
out3sig_dev[i] <- list(reglasigma(ValenBisi_por_tramo[[i]]$numero_de_devoluciones))
out3sig_pres[i] <- list(reglasigma(ValenBisi_por_tramo[[i]]$numero_de_prestamos))
}
...
outham_pres <- list()
for (i in 1:24){
outham_dev[i] <- list(reglahampel(ValenBisi_por_tramo[[i]]$numero_de_devoluciones))
outham_pres[i] <- list(reglahampel(ValenBisi_por_tramo[[i]]$numero_de_prestamos))
}
...
outper_pres <- list()
for (i in 1:24){
outper_dev[i] <- list(reglapercentil(ValenBisi_por_tramo[[i]]$numero_de_devoluciones))
outper_pres[i] <- list(reglapercentil(ValenBisi_por_tramo[[i]]$numero_de_prestamos))
}
```
Observamos que con casi cualquier método (excepción los outliers por precentil), clasifica como valores extremos los valores altos.
```{r}
#paradas con mayor número de devoluciones y préstamos

df_top10_pres <- ValenBisi[order(ValenBisi$tramo_horario, -ValenBisi$numero_de_prestamos), ] %>% 
...
df_top10_dev <- ValenBisi[order(ValenBisi$tramo_horario, -ValenBisi$numero_de_devoluciones), ] %>% 
  group_by(tramo_horario) %>%
  slice_head(n = 10) %>%
...
barplot10_dev <- ggplot(df_top10_dev, aes(x = tramo_horario, y = numero_de_devoluciones, fill = numero_de_devoluciones, text = paste("Número de devoluciones: ", numero_de_devoluciones, "<br>Estación: ", estacion))) +
...
barplot10_pres <- ggplot(df_top10_pres, aes(x = tramo_horario, y = numero_de_prestamos, fill = numero_de_prestamos, text = paste("Número de préstamos: ", numero_de_prestamos, "<br>Estación: ", estacion))) +
  geom_bar(stat = "identity") +
...
barplot_dev <- ggplot(ValenBisi, aes(x = tramo_horario, y = numero_de_devoluciones, fill = numero_de_devoluciones, text = paste("Número de devoluciones: ", numero_de_devoluciones, "<br>Estación: ", estacion))) +
  geom_bar(stat = "identity") +
  labs(title = "Devoluciones por hora", x = "Tramo Horario", y = "Devoluciones") +
  scale_fill_gradient(name = "Número de Devoluciones") +
  theme_minimal() + 
...
barplot_pres <- ggplot(ValenBisi, aes(x = tramo_horario, y = numero_de_prestamos, fill = numero_de_prestamos, text = paste("Número de préstamos: ", numero_de_prestamos, "<br>Estación: ", estacion))) +
  geom_bar(stat = "identity") +
  labs(title = "Préstamos por hora", x = "Tramo Horario", y = "Préstamos") +
  scale_fill_gradient(low = "#66c2a5", high = "#fc8d62", name = "Número de Préstamos") +
  theme_minimal() + 
...
corr1 <- ggplot(ValenBisi) + 
  geom_point(aes(x = numero_de_devoluciones, y = numero_de_prestamos, color = tramo_horario, alpha = 0.5, text = paste("Tramo Horario: ", tramo_horario, "<br>Estación: ", estacion, "<br>Número de devoluciones: ", numero_de_devoluciones, "<br>Número de préstamos: ", numero_de_prestamos))) + 
  labs(title = "Correlación número de préstamos y devoluciones", x = "Número de devoluciones", y = "Número de préstamos", color = "Tramo Horario", alpha = "") +
  geom_smooth(aes(x = numero_de_devoluciones, y = numero_de_prestamos), method = "lm", se = FALSE, color = "blue", size = 0.4, linetype = "dashed")
...
```{r}
#Prueba flujo de personas (filtrar horas)
tram_filt1 = "07H - 08H"
tram_filt2 = "23H - 24H"
Valenbisi_entre_horas <- ValenBisi %>% filter(tramo_horario < tram_filt1 | tramo_horario > tram_filt2)

corr1filt <- ggplot(Valenbisi_entre_horas) + 
  geom_point(aes(x = numero_de_devoluciones, y = numero_de_prestamos, color = tramo_horario, alpha = 0.5, text = paste("Tramo Horario: ", tramo_horario, "<br>Estación: ", estacion, "<br>Número de devoluciones: ", numero_de_devoluciones, "<br>Número de préstamos: ", numero_de_prestamos))) + 
  labs(title = "Correlación número de préstamos y devoluciones desde las 00:00 a las 07:00", x = "Número de devoluciones", y = "Número de préstamos", color = "Tramo Horario", alpha = "") +
  geom_smooth(aes(x = numero_de_devoluciones, y = numero_de_prestamos), method = "lm", se = FALSE, color = "blue", size = 0.4, linetype = "dashed")
  

ggplotly(corr1filt, tooltip = "text")
```
...
Paradas_bici<-ValenBisi_disponibilidad%>%
  separate(geo_point_2d, into=c("latitud_y","longitud_x"), sep=",")%>% #Hago separate para crear las columnas latitud y longitud
  mutate(latitud_y = as.numeric(latitud_y))%>% # Pasar las columnas a numérico para poder utilizarlas mas adelante
  mutate(longitud_x = as.numeric(longitud_x))

```

Ahora creamos el mapa general


```{r}
map <- leaflet() %>%
  addTiles() %>%  
  addMarkers(data = na.omit(Paradas_bici), ~longitud_x, ~latitud_y, label = ~Direccion)
# Print the map
print(map)
```

Hago los cabios a base de datos completa
...
Para tener un ejemplo de coloreo, voy a tomar el tramo 14-15 h y voy a hacer el mapa con las estaciones, con el color del número de préstamos que recibe cada una:

```{r}
color_pal <- colorNumeric(palette = "Blues", domain = ValenBisi$numero_de_prestamos)
...
map <- leaflet(tramo_14_15) %>%
  ...
  addCircleMarkers(~longitud_x, ~latitud_y, label = ~Direccion,
                   color = "black",
                   weight = 1,
                   fillColor = ~color_pal(numero_de_prestamos),  # ajusta el tamaño de los círculos según tus necesidades
                   radius = 5,
                   fillOpacity = 1,
...
Veo que hay muchas paradas con muy poca concurrencia, y unas pocas con mucha, por lo que hago la raiz cuadrada a los datos para suavizarlos:
...
map <- leaflet(tramo_14_15) %>%
  ...
  addCircleMarkers(~longitud_x, ~latitud_y, label = ~Direccion,
                   color = "black",
                   weight = 1,
                   fillColor = ~color_pal(numero_de_prestamos^0.5),  # ajusta el tamaño de los círculos según tus necesidades
                   radius = 5,
                   fillOpacity = 1,
                   popup = ~as.character(numero_de_prestamos))%>%
...
creación de los datos por tramo:
...
```{r}
corr1_coef_spear <- cor(x = ValenBisi$numero_de_devoluciones, y = ValenBisi$numero_de_prestamos, use = "complete.obs", "spearman")
corr1_coef_spear

corr1_coef_pear <- cor(x = ValenBisi$numero_de_devoluciones, y = ValenBisi$numero_de_prestamos, use = "complete.obs", "pearson")
corr1_coef_pear #correlación lineal (< 0.8)

cov1_coef_spear <- cov(x = ValenBisi$numero_de_devoluciones, y = ValenBisi$numero_de_prestamos, use = "complete.obs", "spearman")
cov1_coef_spear
```
```{r}
#plot balance préstamos/devoluciones por hora (todas las estaciones)
Balance_Total_plot <- ggplot(TablaEsta, aes(x = tramo_horario, y = Balance_Total, fill = factor(sign(Balance_Total)))) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("red", "blue"), name = "", labels = c("Devoluciones", "Préstamos")) +
...
```
Han desaparecido 2586 bicicletas.¿¿??
