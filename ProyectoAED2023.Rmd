...
interactive_boxplot2 <- plot_ly(ValenBisi, x = ~get(initial_x), 
  ...
  output$map <- renderLeaflet({
    ...
    leaflet() %>%
      ...
      addCircleMarkers(data = selected_df,~longitud_x, ~latitud_y, label = ~Direccion,
                   color = "black",
                   weight = 1,
                   fillColor = ~color_pal(numero_de_devoluciones^0.5),  # ajusta el tamaño de los círculos según tus necesidades
                   radius = 5,
                   fillOpacity = 1,
...
head(ValenBisi)
```
Y el mapa de las paradas por metro mas cercano
...
map <- leaflet(tramo_14_15) %>%
  ...
  addCircleMarkers(~longitud_x, ~latitud_y, label = ~Direccion,
                   color = "black",
                   weight = 1,
                   fillColor = ~color_pal(distancia_metro_cercano^0.5),  # ajusta el tamaño de los círculos según tus necesidades
                   radius = 5,
                   fillOpacity = 1,
...
```
No vemos que haya una correlación, pero tiene sentido que en las zonas que tienen metros cerca, haya mas afluencia de gente, por lo que también se use mas el valenbisi.
